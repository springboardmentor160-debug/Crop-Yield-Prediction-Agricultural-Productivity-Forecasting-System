...
class AnalyticsService:
    ...
    def dashboard_summary(db: Session, user_id: int) -> dict:
        summary = AnalyticsService.dashboard(db, user_id)
        yield_trend = AnalyticsService.yield_trends(db, user_id).get("yield_trend", [])
        productivity = AnalyticsService.productivity(db, user_id)
        farm_comparison = AnalyticsService.farm_comparison(db, user_id).get("farms", [])

        seasonal_comparison = []
        if len(yield_trend) >= 2:
            seasonal_comparison = [
                {"period": yield_trend[-2]["month"], "avg_yield": yield_trend[-2]["avg_yield"]},
                {"period": yield_trend[-1]["month"], "avg_yield": yield_trend[-1]["avg_yield"]},
            ]
        elif len(yield_trend) == 1:
            seasonal_comparison = [
                {"period": yield_trend[0]["month"], "avg_yield": yield_trend[0]["avg_yield"]},
            ]

        return {
            "productivity_score": productivity.get("productivity_score", 0),
            "productivity_rating": productivity.get("performance_rating", "Unknown"),
            "estimated_production_tons": productivity.get("estimated_production_tons", 0),
            "estimated_revenue_inr": productivity.get("estimated_revenue_inr", 0),
            "yield_trend": yield_trend,
            "seasonal_comparison": seasonal_comparison,
            "farm_comparison": farm_comparison,
            "summary_metrics": {
                "total_farms": summary.get("total_farms", 0),
                "total_predictions": summary.get("total_predictions", 0),
                "average_yield_tons_per_ha": summary.get("average_yield_tons_per_ha", 0),
                "high_risk_predictions": summary.get("high_risk_predictions", 0),
            },
        }
...
class AnalystService:
    ...
    def dashboard(db: Session) -> dict:
        ...
        return {
            "coverage_farms": total_farms,
            "active_reports": total_predictions,
            "insights_count": len(crops_analyzed),
    ...
    @staticmethod
    def reports(db: Session, days: int = 30) -> list:
        ...
        return [
            {
                "id": 1,
                "title": f"Platform Report — Last {days} Days",
                "created_at": datetime.utcnow().isoformat(),
                "summary": f"Analyzed {len(predictions)} predictions across {len(set(p.farm_id for p in predictions))} active farms. Average yield: {avg_yield} t/ha.",
            }
        ]
