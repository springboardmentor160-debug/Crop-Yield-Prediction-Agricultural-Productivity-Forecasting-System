...
import type { Farm, SoilAnalysisOut } from "@/lib/types";

export default function SoilPage() {
  const router = useRouter();
  const [result, setResult] = useState<SoilAnalysisOut | null>(null);
          ...
          <div>
            {!result ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                ...
                {/* Fertility Score */}
                <div style={{ backgroundColor: "#1a1000", border: `1px solid ${fertilityColor(result.fertility_index || "Medium")}40`, borderRadius: "16px", padding: "24px" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                    <div>
                      <div style={{ fontSize: "52px", fontWeight: 900, color: fertilityColor(result.fertility_index || "Medium"), lineHeight: 1 }}>{result.fertility_score}</div>
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ fontSize: "20px", fontWeight: 800, color: fertilityColor(result.fertility_index || "Medium"), backgroundColor: `${fertilityColor(result.fertility_index || "Medium")}15`, borderRadius: "8px", padding: "8px 16px" }}>
                        {result.fertility_index || "Medium"}
                      </div>
                    </div>
                  </div>
                ...
                <div style={{ backgroundColor: "#1a1000", border: "1px solid #2a1e00", borderRadius: "16px", padding: "24px" }}>
                  ...
                  {result.nutrients && Object.entries(result.nutrients).map(([nutrient, data]: [string, { value: number; unit: string; status: string }]) => (
