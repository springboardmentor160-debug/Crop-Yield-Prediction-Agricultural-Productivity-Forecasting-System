...
def delete_user(
    user_id: int,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(require_permission("delete_any_user"))
):
...
def update_user_role(
    ...
    role_data: dict,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(require_permission("change_user_roles"))
):
    user = db.query(models.User).filter(models.User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def get_all_farms(
    db:Session=Depends(get_db),
    current_user:models.User=Depends(require_permission("view_all_farms"))
):
...
def get_predictions(
    db:Session=Depends(get_db),
    current_user:models.User=Depends(require_permission("view_all_predictions"))
):
...
def delete_prediction(
    prediction_id:int,
    db:Session=Depends(get_db),
    current_user:models.User=Depends(require_permission("delete_any_prediction"))
):
    prediction=db.query(models.Prediction).filter(
        models.Prediction.id==prediction_id
    ).first()
...
@router.get("/system")
def system_health(
    current_user:models.User=Depends(require_permission("system_health"))
):
    return {
        "api":"Running",
        "database":"Connected",
...
@router.get("/model")
def model_metrics(
    current_user:models.User=Depends(require_permission("full_model_access"))
):
...
def export_admin_csv(
    db: Session = Depends(get_db),
    current_user: models.User = Depends(require_permission("export_reports"))
):
    output = StringIO()
